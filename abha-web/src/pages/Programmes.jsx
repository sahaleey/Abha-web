...
const ProgrammeCard = ({ item }) => {
  ...
  return (
    ...
    >
      ...
      <motion.div
        layout
        className={`transition-all duration-100 px-4 pt-3 pb-4 ${
          expanded ? "block opacity-100" : "hidden opacity-0"
        }`}
      >
        <h3 className="text-lg font-bold text-white">{item.name}</h3>
        <p className="text-sm text-gray-400">{item.stage}</p>
        <p className="text-sm text-gray-400">Host: {item.host}</p>
...
const Programmes = () => {
  return (
    <div className="min-h-screen mt-17 font-bloomsburg p-4 sm:p-6 md:p-10 flex flex-col items-center gap-10 relative">
      <div className="absolute top-0 left-0 right-0 bottom-0 to-black opacity-50 -z-20"></div>
      ...
      <div className="flex flex-col xl:flex-row gap-10 items-center xl:items-start w-full justify-center relative z-10">
        <ProgrammeBox title="Already Done" programmes={alreadyDoneProgrammes} />
